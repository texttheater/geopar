...
class ItemsTestCase(unittest.TestCase):
    ...
    def test_items_example1(self):
        ...
        item = item.skip()
        item = item.skip()
        item = item.shift(1, terms.string2term('capital(_, _)'))
        item = item.coref(0, 1, 0, 2)
        item = item.drop(0, 2)
        item = item.skip()
        item = item.skip()
        item = item.shift(1, terms.string2term('state(_)'))
        item = item.coref(1, 1, 0, 1)
        item = item.skip()
        item = item.skip()
        item = item.shift(1, terms.string2term('largest(_, _)'))
        item = item.lift(0, 2)
        item = item.shift(1, terms.string2term('population(_, _)'))
        item = item.coref(1, 1, 0, 1)
        item = item.coref(0, 1, 0, 2)
        item = item.drop(0, 2)
        item = item.drop(0, 2)
        ...
        self.assertTrue(item.stack[0].equivalent(terms.string2term(
